...
//complex<double> Triplet::ComputeBisError(Target & target, UVPoint uv_ab, UVPoint uv_bc, UVPoint uv_ac)
//{
//    /// \bug Bispectrum error is set to 0.001 by default
//    return complex<double>(0.001, 0.001);
//}

complex<double> Triplet::ComputeBis(Target & target, UVPoint uv_ab, UVPoint uv_bc, UVPoint uv_ac)
{
...
double  Triplet::GetBisAmp(Target & target, UVPoint uv_ab, UVPoint uv_bc, UVPoint uv_ac)
{
    complex<double> bis = GetBispectra(target, uv_ab, uv_bc, uv_ac);
    return abs(bis);
}

// Wrapper function for the UV-coordinate taking versions.
double  Triplet::GetBisAmp(Target & target, double hour_angle, double wavenumber)
{
    ...
    return GetBisAmp(target, uv_ab, uv_bc, uv_ac);
}

// Returns the norm (amplitude) of the bispectrum
double  Triplet::GetBisAmpErr(Target & target, UVPoint uv_ab, UVPoint uv_bc, UVPoint uv_ac)
{
...
// Wrapper function for the UV-coordinate taking versions.
double  Triplet::GetBisAmpErr(Target & target, double hour_angle, double wavenumber)
{
    UVPoint uv_ab = mBaselines[0]->UVcoords(hour_angle, target.declination);
    uv_ab.Scale(wavenumber);
    UVPoint uv_bc = mBaselines[1]->UVcoords(hour_angle, target.declination);
    ...
    uv_ac.Scale(wavenumber);
     ...
     return GetBisAmpErr(target, uv_ab, uv_bc, uv_ac);
}


// Wrapper function for the UV-coordinate taking versions.
double  Triplet::GetBisPhi(Target & target, double hour_angle, double wavenumber)
{
    UVPoint uv_ab = mBaselines[0]->UVcoords(hour_angle, target.declination);
    uv_ab.Scale(wavenumber);
    UVPoint uv_bc = mBaselines[1]->UVcoords(hour_angle, target.declination);
    ...
    return GetBisPhi(target, uv_ab, uv_bc, uv_ac);
}

/// Returns the argument (phase) of the bispectra
double  Triplet::GetBisPhi(Target & target, UVPoint uv_ab, UVPoint uv_bc, UVPoint uv_ac)
{
    complex<double> bis = GetBispectra(target, uv_ab, uv_bc, uv_ac);
    return arg(bis);
}
...
// Wrapper function for the UV-coordinate taking versions.
double  Triplet::GetBisPhiErr(Target & target, double hour_angle, double wavenumber)
{
    UVPoint uv_ab = mBaselines[0]->UVcoords(hour_angle, target.declination);
    uv_ab.Scale(wavenumber);
    UVPoint uv_bc = mBaselines[1]->UVcoords(hour_angle, target.declination);
    ...
    return GetBisPhiErr(target, uv_ab, uv_bc, uv_ac);
}

double  Triplet::GetBisPhiErr(Target & target, UVPoint uv_ab, UVPoint uv_bc, UVPoint uv_ac)
{
    /// \bug Returns 0.001 by default.
    return 0.001;
}   
...
complex<double> Triplet::GetBispectra(Target & target, UVPoint uv_ab, UVPoint uv_bc, UVPoint uv_ac)
{
    ...
    else
    {
        ...
        bis = ComputeBis(target, uv_ab, uv_bc, uv_ac);
...
complex<double> Triplet::GetBispectra(Target & target, double hour_angle, double wavenumber)
{
	UVPoint uv_ab = mBaselines[0]->UVcoords(hour_angle, target.declination);
	uv_ab.Scale(wavenumber);
	UVPoint uv_bc = mBaselines[1]->UVcoords(hour_angle, target.declination);
	...
	return GetBispectra(target, uv_ab, uv_bc, uv_ac);
}
